...
def compute_rank_score(quiz: float, assignment: float, completion: float, streak: float, weights: Optional[dict] = None) -> float:
    """Calculate overall rank score using weighted formula.

    Accepts an optional `weights` mapping or sequence as a fifth argument to
    maintain compatibility with callers/tests that pass five positional args.
    ...
    Args:
        ...
        completion: Course completion score (0-100)
        streak: Streak score (0-100)
        weights: Optional mapping (or sequence) to override default WEIGHTS.
                 If a sequence is provided, it's interpreted as [quiz, assignment, completion, streak].
    ...
    if weights is not None:
        # Normalize weights if caller provides a sequence (list/tuple)
        if isinstance(weights, (list, tuple)):
            ...
            # overlay provided keys onto defaults
            used = {**WEIGHTS, **weights}

    return round(
        quiz * used["quiz"] + assignment * used["assignment"] +
        completion * used["completion"] + streak * used["streak"], 2
    )