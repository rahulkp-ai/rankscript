...
from sqlalchemy import (
    Column, DateTime, ForeignKey,
    Float, Integer, Boolean, UniqueConstraint, Uuid, String
)
from sqlalchemy.orm import relationship
import uuid
from app.db.base import Base
...
class Enrollment(Base):
    ...
    progress        = Column(Float, default=0.0)       # 0.0 – 100.0 percent
    lessons_done    = Column(Integer, default=0)
    # Lesson IDs are strings in the application/tests (e.g. "lesson-1"). Store as text.
    last_lesson_id  = Column(String(255), nullable=True)
